...
async def get_quiz(
    ...
    return schemas.QuizOut(
        ...
        youtube_id=quiz.video.youtube_id,  # Make sure we return the YouTube ID
        questions=out_qs
    )
...
async def get_quiz_by_youtube_id(
    ...
    user_exists_query = select(models.User).where(models.User.id == user_id)
    user_result = await db.execute(user_exists_query)
    user = user_result.scalar_one_or_none()
    
    # Create test user if needed (in development)
    if not user and user_id == "test123":
